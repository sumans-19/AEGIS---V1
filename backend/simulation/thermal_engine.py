...
def generate_thermal_frame(drone: DroneState, world: WorldState) -> bytes:
    ...
    for ix in range(max(0, gx_center - view_cells), min(20, gx_center + view_cells)):
        for iy in range(
            max(0, gy_center - view_cells), min(20, gy_center + view_cells)
        ):
            ...
            if 0 < sx < WIDTH and 0 < sy < HEIGHT:
                cv2.rectangle(
                    arr, (sx - 15, sy - 15), (sx + 15, sy + 15), float(cell_temp), -1
                )
    ...
    cv2.putText(
        colored,
        f"THERMAL | {drone.callsign} | AMB {world.ambient_temp:.1f}C",
        (10, 20),
...
def generate_camera_frame(drone: DroneState, world: WorldState) -> bytes:
    ...
    for i in range(max(0, gx_center - 4), min(20, gx_center + 4)):
        for j in range(max(0, gy_center - 4), min(20, gy_center + 4)):
            ...
            if damage > 0.8:
                fill = (42, 26, 10)  # Rubble
            elif damage > 0.4:
                fill = (58, 42, 26)  # Damaged

            draw.rectangle(
                [px - 18, py - 18, px + 18, py + 18],
                fill=fill,
                outline=(100, 100, 100, 50),
            )
    ...
    draw.text(
        (10, 215),
        f"DRONE-0{drone.id} {drone.callsign} | ALT {drone.pos[1]:.0f}m | SPD {np.linalg.norm(drone.vel):.1f}m/s",
        fill=(0, 229, 255),
    )
    ...
    for s in world.survivors:
        ...
        if abs(dx) < 20 and abs(dy) < 15:  # in frame approx
            ...
            if s.detected and s.id in world.detected_survivors:
                draw.rectangle(
                    [sx - 10, sy - 10, sx + 10, sy + 10], outline=(0, 255, 136), width=2
                )
                draw.text(
                    (sx + 12, sy - 5),
                    f"S#{s.id} {s.confidence:.0%}",
                    fill=(0, 255, 136),
                )
